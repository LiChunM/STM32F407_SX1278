...
void RTC_Set(u16 year,u8 month,u8 date,u8 hour,u8 min,u8 sec)
{
	...
	RTC_Set_Date(year-2000,month,date,RTC_Get_Week(year,month,date));
}
...
{
	u8 ampm;
	RTC_Get_Time(&calendarx->hour,&calendarx->min,&calendarx->sec,&ampm);
}
...
void SysTimeSync(u8 *timebuf)
{
	...
	u8 tempbuf[14]={0};
	u32 temptime;
	for(i=0;i<14;i++)tempbuf[i]=timebuf[10+i]-0x30;
	temptime=tempbuf[0]*1000+tempbuf[1]*100+tempbuf[2]*10+tempbuf[3];
	calendar.w_year=temptime;
	temptime=tempbuf[4]*10+tempbuf[5];
	calendar.w_month=temptime;
	temptime=tempbuf[6]*10+tempbuf[7];
	calendar.w_date=temptime;
	temptime=tempbuf[8]*10+tempbuf[9];
	calendar.hour=temptime;
	temptime=tempbuf[10]*10+tempbuf[11];
	calendar.min=temptime;
	temptime=tempbuf[12]*10+tempbuf[13];
	calendar.sec=temptime;
	RTC_Set(calendar.w_year,calendar.w_month,calendar.w_date,calendar.hour,calendar.min,calendar.sec);
	if(SystemDebug==2)printf("+time=%04d-%02d-%02d %02d:%02d:%02d\r\n",calendar.w_year,calendar.w_month,calendar.w_date,calendar.hour,calendar.min,calendar.sec);
}
